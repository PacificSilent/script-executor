...
app.post('/execute', (req, res) => {
  ...
  WshShell.Run "cmd.exe /c ${path.join(__dirname, 'scripts', 'global_fps_custom.bat')} ${fps}", 0, False`;
    ...
    proceso.on('close', (code) => {
      ...
      if (code === 0) {
        res.json({ message: `Script "${scriptConfig.description}" executed successfully`, output });
      } else {
        res.status(500).json({
  ...
  if (!fs.existsSync(vbsScriptPath)) {
    return res.status(404).json({ message: 'VBScript file for the script does not exist' });
  }
